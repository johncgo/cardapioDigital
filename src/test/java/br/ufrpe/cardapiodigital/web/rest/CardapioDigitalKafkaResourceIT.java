package br.ufrpe.cardapiodigital.web.rest;
...
import br.ufrpe.cardapiodigital.IntegrationTest;
import br.ufrpe.cardapiodigital.config.EmbeddedKafka;
import br.ufrpe.cardapiodigital.config.KafkaSseConsumer;
import br.ufrpe.cardapiodigital.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class CardapioDigitalKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/cardapio-digital-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/cardapio-digital-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/cardapio-digital-kafka/unregister"));
                return;
            }
        }
